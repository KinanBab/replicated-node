...
Client.prototype._accept = function (connection) {
  ...
  this._connection.on('ping', function(cancel, data) { console.log('ping'); cancel(); });
...
/*
var client = new Client(process.argv[2], 'localhost:3000');
client.on('error', console.log);
client.on('connectError', function () { console.log.call(console, 'failed', ...arguments); });
client.on('close', function () { console.log('close', ...arguments); });
client.on('connect', function () { client.sendString('hello'); });
client.on('receiveString', console.log);
client.on('receiveBuffer', console.log);
client.connect();
*/
