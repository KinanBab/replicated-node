 ...
 */
function CommunicationManager(system) {
  var me = system.config('id');
  var port = system.config('port');
  var servers = system.config('remotes');
...
CommunicationManager.prototype._receiveRPC = function (id, message) {
  ...
  if (action === 1) {
    ...
    func.call(null, ret_func, id, ...parameters);
  }
